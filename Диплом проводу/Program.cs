...
namespace ConsoleApplication
{
    class Program
    {
        ...
        private const string OutputFileName = "result.csv";
        ...
        { 
            ...
            HandleLastGroupIfQueueIsNotEmpty(extendedQueue, columns.Length);
        }
        ...
        private static void HandleGroup(string[][] rows)
        {
            ...
            for (var i = 0; i < columns.Length; i++)
            {
                if (_columnsToShuffleIndicies.Contains(i))
                    tempColumns[i] = DataShuffler.ShuffleColumn(columns[i], _seed1, _seed2, ChunkSize);
                else
        ...
        private static int[] AskUserAndGetColumnsToShuffleIndicies(string[] columns)
        {
            ...
            Console.WriteLine($"Введите через пробел названия столбцов, которые нужно перемешать");
        ...
        private static void HandleLastGroupIfQueueIsNotEmpty(ExtendedQueue extendedQueue, int columnsCount)
        {
            if (extendedQueue.Count != 0)
            {
                var emptyCellsCount = ChunkSize - extendedQueue.Count;

                for (var i = 0; i < emptyCellsCount; i++)
                    extendedQueue.Enqueue(Enumerable.Range(0, columnsCount).Select(x => "Out of data").ToArray());
            }
        }
    }
}